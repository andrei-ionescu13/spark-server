...
export class DuplicateArticleUseCase implements UseCase<DuplicateArticleRequestDto, Response> {
  ...
  execute = async (request: DuplicateArticleRequestDto): Promise<Response> => {
    ...
    try {
      ...
      const articleFoundByProps = await this.articleQueryRepo.getArticleByProps([
        { slug: props.slug },
        { title: props.title },
      ]);
      ...
      const articleCreatedOrError = await this.articleCommandRepo.createArticle(articleProps);
